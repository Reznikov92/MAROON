...
2.Фильтрация товаров на странице каталога (у нас только главная, но можно заложить логику).
3.Модальное окно для товара (подробнее).
4.Валидация формы (в контактах или подписке).
5.Бургер-меню для мобильной версии.
6.Плавная прокрутка или анимации при скролле.
7.Подсветка активных ссылок.
8.Отправка данных форм (подписка, анкета) без перезагрузки.

1.Нужно повести сайт через валидатор
2.Кроссообр. на браузерах
3.Каталог и продукты подключить.
4.Сделать список товаров в отдельный файл